...
Last seeded: 2026-05-14. Last updated: 2026-07-15 (added Cohort 6 — Template Functional Loop; demoted Cohort 5 per the 2026-07-15 weekly; later same day added Cohort 7 — Template Engine v2 / Heidi parity, plan docs/plans/template-engine-v2.md, queued behind Cohort 6). Prior: 2026-07-01 (added Cohort 5 — Grounded AI Scribe: #620–#625; see memory/grounded-scribe-gap-map.md). Prior: 2026-05-30 (seeded 17 backend issues from GitHub per CTO's "implement all backend" directive).
...
### Cohort 7 — Template Engine v2 / Heidi parity — ⛔ ON HOLD: planning approved, implementation NOT authorized (Faïçal, 2026-07-15 "don't implement yet"). Do NOT pick up ANY TPL-V2 item — even with a free lane and empty cohorts above — until Faïçal explicitly green-lights in the backlog. Plan: docs/plans/template-engine-v2.md
This is the CONTINUATION of Cohort 6's "Développer moteur modèles" once the functional loop closes — per Faïçal's 2026-07-15 weekly (finish selection+formatting first) AND his same-day Heidi-parity feasibility sign-off. Complements loop-2 (Library SOAP seed) and loop-4 (change-template picker) — do not duplicate them. Goal: Heidi-class template authoring (content slots, style directives, verbatim blocks, live example preview, generate-from-description) on Aurion's grounded pipeline — every slice keeps backend/web parity; grounding stays code-enforced (template directives are style/structure-scoped ONLY; sanitizer + runtime validators). Generation changes dark behind `template_engine_v2_enabled` + eval receipt before flip. Marketplace + PDF-form templates are explicitly deferred — do not pick up.
- [ ] TPL-V2-0 · Template schema v2 (content_slots/style/verbatim_blocks) + save-time sanitizer + caps — 2d — lane: backend — no blockers
- [ ] TPL-V2-1 · source_type "template" provenance: assembly-injected verbatim claims + citation expansion + web "Template" chip (+iOS chip, bundled build) — 2d — lane: backend (+web/ios chips) — depends on TPL-V2-0
- [ ] TPL-V2-2 · Stage-1 prompt rendering behind template_engine_v2_enabled (flag full-stack: schema/admin/terraform/portal toggle) + byte-identical-off golden tests + GS-9-style eval receipt — 3d — lane: backend — depends on TPL-V2-0
- [ ] TPL-V2-3 · Authoring editor parity: chat emits v2 fields (sanitizer errors → correction re-prompt), draft card renders slots/style/verbatim, read-only Heidi-style Structure text view, EN+FR — 3d — lane: web (+backend authoring prompt) — depends on TPL-V2-0
- [ ] TPL-V2-4 · Live example preview: sandboxed POST /me/template-authoring/{id}/preview vs bundled synthetic transcripts (no session/note rows, rate-limited) + web Example tab with specialty switcher, EN+FR — 3d — lane: backend (+web tab) — depends on TPL-V2-2, TPL-V2-3
- [ ] TPL-V2-5 · Picker upgrades: GET /me/sessions/{id}/template-suggestion (expose existing resolution ladder w/ reason) + web picker "Generate from description…" (pre-seeded authoring chat) + Suggested row, EN+FR — 2d — lane: web (+backend endpoint) — depends on TPL-V2-3
