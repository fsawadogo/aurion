...
- [ ] TE-1 · Detail level — verbosity under template/session control (+ wire the Brief/Standard/Detailed rail control already designed in #662) — 5d — lane: backend (+web) — no blockers — kills the hardcoded maximalist directive; the direct fix for "trop verbeuse". Carries **web parity in-slice** per Faïçal's v2 rule: *"a backend field nobody can author or see doesn't count as shipped."*
...
- [ ] TE-3 · Template-aware frame capture (**the root fix**) — 3d — lane: backend — depends on TE-2 — pass the target section's guidance + encounter context into the caption prompt so the model describes what the section needs. **Clinician text reaching a VISION prompt must be banlist-screened (`prompts/safety.py`) + fenced under "describe only what is literally visible"** — adopted from Faïçal's v2 safety model; a hostile template may degrade style, never grounding.
- [ ] TE-3b · Portal toggle for `template_engine_enabled` — 1d — lane: backend (web/**) — depends on TE-3 — the admin Feature Flags page is a hardcoded `FLAG_GROUPS` array, NOT generic (my TE-3 plan claimed otherwise and was wrong). Until this lands the flag needs a direct AppConfig write, so **it blocks the rollout's "flip it in dev" step**. Needs `FLAG_GROUPS` entry + `web/types` + en/fr strings.
- [ ] TE-4 · Template-formatted frame merge (replace the verbatim paste) — 3d — lane: backend — depends on TE-2 — route by the template's sections; precedents: `measurement/note_injection.py`, `screen.py:_value_to_claim_text`
      **+ carries a TE-3 defect (found in TE-3's second review, pinned in `::test_tier3_prediction_and_placement_can_disagree`):** tier 3 of `_find_target_section` makes prediction and placement DISAGREE — `merge_visual_citations` flips `pending_video`→`populated` as it goes and REPEATS are dropped between the two calls, so on a custom template whose ids fall outside the hardcoded tier-2 tuple (i.e. the templates this epic is FOR) two frames get captured under one section's guidance and filed under two. Routing on the template's own sections is what fixes it — do not ship TE-4 without closing this.
...
- [ ] loop-2 · Seed the Library with SOAP (starter_library/soap.json + idempotent scripts/seed_library.py, is_shared=True) — 2d — lane: backend — no blockers — closes "Localiser modèle SOAP"; source = Marie's 2026-06-12 S/O/A/P notes. **NEXT unblocked backend item.**
