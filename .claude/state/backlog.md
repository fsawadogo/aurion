...
- [ ] TE-4b · Generation + regeneration progress feedback — 1d — lane: web — no blockers — **found by Uzziel on the first real prod upload, 2026-07-20.** Two gaps: (1) the upload stepper's active stage is a STATIC dot, so a slow stage is indistinguishable from a hung page — that is why a working import "seemed stuck"; (2) on note review, regeneration shows only a small grey caption while **the note body keeps rendering the superseded note at full opacity and Print stays enabled** — a clinician can print/copy a note mid-replacement and get the old version. That half is a correctness bug, not polish. Polling failure is already handled (`lib/poll.ts`, 5 consecutive errors → "lost contact"); this is about showing WORK IN PROGRESS.
