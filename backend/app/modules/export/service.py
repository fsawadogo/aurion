...
def _export_footer() -> str:
    """Mode-aware export footer (GS-9 labelling, scribe-0 #620).

    Descriptive mode (flag OFF) keeps the exact prior wording — byte-identical.
    Grounded Synthesis Mode labels the note as a source-grounded AI note rather
    than a "descriptive record", so a synthesized note is never shipped
    mislabelled as descriptive.
    """
    if get_config().feature_flags.grounded_synthesis_enabled:
        return (
            "Generated by Aurion Clinical AI. This is an AI-generated clinical "
            "note; every statement is grounded in a cited source. Clinician "
            "review required before clinical use."
        )
    return (
...
async def export_note_docx(
    ...
    footer_run = footer_para.add_run(_export_footer())
