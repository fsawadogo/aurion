...
_STYLE: dict[str, str] = {
    ...
    "orthopedic_surgery": (
        ...
        "the right') — always the EXACT test name as stated, never a "
        "similar test ('patellar grind' or 'resisted SLR' must not "
        "become 'straight leg raise'); and laterality (right/left) "
        "wherever mentioned. "
...
_GROUNDED_STYLE: dict[str, str] = {
    "orthopedic_surgery": (
        ...
        "physician distinguishes them); strength on the 0-5 scale when "
        "graded (e.g. '4/5'); named special tests with their result and "
        "side (e.g. 'Lachman negative on the left'); and laterality "
        "wherever mentioned. Imaging review must note modality + laterality "
        "+ view. In the assessment you MAY synthesize the working diagnosis "
        "and plan from the captured findings, but every assessment statement "
        "must cite the exam/imaging/transcript source(s) it rests on — never "
        "state a diagnosis or differential the cited findings do not support."
    ),
    "plastic_surgery": (
        "Style: document wounds with the standard assessment fields the "
        "physician states — anatomical location; dimensions as length × "
