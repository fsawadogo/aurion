...
async def resolve_session_template(
    session: SessionModel, db: AsyncSession
) -> Template:
    """The ``Template`` the session's CURRENT note was built with (TE-2).
    ...
    "Current" is load-bearing and depends on every note-writing path keeping
    the pin honest. ``regenerate_note`` persists an explicitly-switched
    template back onto the session for exactly this reason; if a future path
    rebuilds a note under a different template WITHOUT updating the pin, this
    function starts lying and Stage 2 aims at the wrong sections.
    ...
    ``session`` must be a real row — ``None`` is out of contract (callers
    holding a ``scalar_one_or_none()`` result must guard first). A deleted or
    unparseable custom pin degrades to the built-in / specialty path instead
    of raising, so a stale binding alone can never break the caller.
    """
    # Lazy import: session.service imports note_gen.service in three places
    # (its own lazy imports), so a module-level import here would risk a cycle
    # the moment either side grows a top-level dependency. Mirrors the
    # custom_templates import inside _resolve_stage1_template below.
...
async def generate_stage1_note(
    ...
    # The critique pass is an Anthropic tool-call implementation — only run it
    # when this note was actually generated by Anthropic. This stops a hidden
    # Anthropic call from firing when the configured note-gen provider is
