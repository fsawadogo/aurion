...
from app.modules.prompts.assembly import _compose_system_prompt
...
def test_grounded_template_override_is_appended_not_substituted(grounded_on):
    ...
    out = _compose_system_prompt(_JOB, None, descriptive, None)
...
def test_grounded_publication_is_appended(grounded_on):
    ...
    out = _compose_system_prompt(_JOB, None, None, "PUBLISHED_TEXT")
    assert out.startswith(NOTE_GEN_GROUNDED_SYSTEM_PROMPT)
    assert "PUBLISHED_TEXT" in out


def test_grounded_personal_override_wins_the_appended_layer(grounded_on):
    out = _compose_system_prompt(_JOB, "PERSONAL", "TEMPLATE", "PUBLISHED")
    assert out.startswith(NOTE_GEN_GROUNDED_SYSTEM_PROMPT)
...
def test_grounded_no_override_is_boundary_only(grounded_on):
    assert _compose_system_prompt(_JOB, None, None, None) == NOTE_GEN_GROUNDED_SYSTEM_PROMPT


def test_grounded_flag_does_not_affect_non_note_gen(grounded_on):
    # AC-6: grounded mode only changes note_generation composition.
    pid = next(
        (p for p in ("vision_frame", "vision_reconcile") if p in PROMPTS), None
    )
    if pid is None:
        pytest.skip("no non-note-gen prompt available to compare")
    assert _compose_system_prompt(pid, None, "TEMPLATE_X", None) == "TEMPLATE_X"
...
def test_off_personal_override_replaces(grounded_off):
    assert (
        _compose_system_prompt(_JOB, "PERSONAL", "TEMPLATE", "PUBLISHED") == "PERSONAL"
    )


def test_off_template_replaces_when_no_personal(grounded_off):
    assert _compose_system_prompt(_JOB, None, "TEMPLATE", "PUBLISHED") == "TEMPLATE"


def test_off_published_replaces_when_no_personal_or_template(grounded_off):
    assert _compose_system_prompt(_JOB, None, None, "PUBLISHED") == "PUBLISHED"


def test_off_default_is_descriptive_base(grounded_off):
    assert _compose_system_prompt(_JOB, None, None, None) == NOTE_GEN_SYSTEM_PROMPT
