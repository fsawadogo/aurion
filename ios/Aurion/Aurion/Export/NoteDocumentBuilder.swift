...
enum NoteDocumentBuilder {
    ...
    private static func buildDocumentXML(note: NoteResponse, sessionId: String) -> String {
        ...
        body += para(specialtyTitle(note.specialty), bold: true, size: 44, color: navyHex, after: 0)
        let pct = Int((note.completenessScore * 100).rounded())
        body += para("\(pct)% complete  ·  v\(note.version)  ·  \(note.providerUsed)",
                     size: 18, color: grayHex, after: 20)
        ...
        body += para("  DRAFT  ·  Descriptive record — clinician review required before clinical use",
                     bold: true, size: 18, color: whiteHex, fill: amberHex, before: 200, after: 60)
        body += para("Generated by Aurion Clinical AI  ·  Session \(sessionId)  ·  v\(note.version)  ·  \(note.providerUsed)",
                     size: 16, color: grayHex, after: 20)
        body += para("This document is a descriptive record of the clinical encounter. It contains no diagnostic or interpretive conclusions.",
