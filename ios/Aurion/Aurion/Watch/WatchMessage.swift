...
///
/// IMPORTANT — shared by BOTH targets. This file lives in the iOS app's
/// synchronized group so the phone-side bridge compiles today; when the
/// `AurionWatch` watchOS target is created in Xcode, add this file to its
/// Target Membership (File Inspector ▸ Target Membership ▸ ☑ AurionWatch)
/// so both sides decode the identical shapes. It imports only Foundation
/// — no UIKit/WatchKit — so it compiles unchanged on watchOS.
///
